    ...
    <footer id="contact" class="bg-white dark:bg-dark-bg py-20 md:py-32 border-t border-gray-100 dark:border-dark-border transition-colors duration-300 mt-auto">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center">
            <img src="img/PIRS-LOGO.PNG" alt="PIRS Logo" class="h-8 w-auto object-contain mb-8 grayscale opacity-50" onerror="this.outerHTML='<span class=\'font-bold text-xl text-gray-300 dark:text-dark-text block mb-8\'>PIRS EASTERN</span>'">
            
            <div class="flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 md:space-x-12 mb-12 text-sm text-gray-500 dark:text-dark-textSecondary">
                <span class="hover:text-brand-black dark:hover:text-dark-text transition-colors cursor-pointer">info@pirs.co.id</span>
                <span class="hover:text-brand-black dark:hover:text-dark-text transition-colors cursor-pointer">+62 81359 646964</span>
                <span class="hover:text-brand-black dark:hover:text-dark-text transition-colors cursor-pointer">www.pirs.co.id</span>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Make sure steps code only runs if they exist on the current page
                ...
                steps.forEach((step) => {
                    step.addEventListener('click', () => {
                        steps.forEach(s => {
                            ...
                            if(num) num.className = 'workflow-number w-12 h-12 bg-white dark:bg-dark-surface border border-gray-200 dark:border-dark-border rounded-full flex items-center justify-center text-sm font-bold text-gray-900 dark:text-dark-textSecondary mb-6 shadow-sm transition-all duration-300 group-hover:scale-110';
                        });
                        
                        step.classList.remove('opacity-50');
                        step.classList.add('opacity-100');
                        const activeNum = step.querySelector('.workflow-number');
                        if(activeNum) activeNum.className = 'workflow-number w-12 h-12 bg-brand-black dark:bg-dark-text border border-gray-200 dark:border-dark-text rounded-full flex items-center justify-center text-sm font-bold text-white dark:text-dark-bg mb-6 shadow-md transition-all duration-300 scale-110';
                    });
                });
            }
